...
def play_sound(path_to_sound):
    sound = pygame.mixer.Sound(f"assets/soundeffects/{path_to_sound}")
...
def render_button(display, text, font, bold, color, position, clicking, func):
    text = font.render(text, bold, color)
    text_rect = get_text_rect(text)
    #text_rect.height -= 10
    text_rect.center = (position[0]+text_rect.width/2, position[1]+text_rect.height/2)
    ...
    if text_rect.collidepoint(mouse_pos):
        pygame.draw.rect(display, color, (text_rect.x, text_rect.y, text_rect.width, text_rect.height), 1)
        if clicking:
            func()
            clicking = False

def radius_decrease(entity):
    entity.radius -= 1
